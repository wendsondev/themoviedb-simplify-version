...
export function PaginationController({ totalPages, currentPage, maxItems, setPage }: PaginationControllerProps) {
  const pages = Math.ceil(totalPages / maxItems);
  const maxLeft = Math.floor((maxItems - 1) / 2); // max items to left of current page
  const maxRight = totalPages - currentPage > maxLeft ? 0 : maxLeft - (totalPages - currentPage); // max items to right of current page
  ...
  return (
    <div className="w-full flex justify-center gap-2 my-16">
      ...
      <div className="flex overflow-hidden justify-center">
        {
          Array.from({ length: Math.min(maxItems, pages) }).map((_, index) => index + firstPage - maxRight)
            .map((page, index) => {
              return (
                <button
                  key={index}
                  onClick={() => setPage(page)}
                  className={`font-bold py-2 px-4 text-purple-700 ${page === currentPage ? 'text-2xl' : 'text-base hover:text-xl'} hover:text-purple-900`}
                >
                  {page}
                </button>
              );
