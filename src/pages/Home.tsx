...
export function Home() {
  ...
  const filteredMovies =
    genresFiltered.length > 0 && movies
      ? {
          ...movies,
          results: movies.results.filter((movie) =>
            genresFiltered.every((id) => movie.genre_ids.includes(id))
          ),
        }
      : movies;
      ...
      <main className="overflow-hidden">
        <section className="bg-purple-900">
          <div className="max-w-5xl flex flex-col px-4 py-10 text-white mx-auto md:items-center">
            ...
            <span className="mt-9 font-bold text-sm uppercase">filter by:</span>
              ...
              {data?.genres.map((genre) => {
                ...
                return (
                  <button
                    ...
                    className={`flex gap-2 items-center text-gray-800 font-bold py-1.5 px-5 rounded transition-colors hover:bg-orange-700 hover:text-white md:py-2 md:px-4 ${
                      filterIsActive ? 'bg-orange-700 text-white' : 'bg-white'
                    }`}
                  >
                    {genre.name}
                    {filterIsActive && (
                      <AiFillCloseCircle className="text-white" size={18} />
                    )}
                  </button>
                );
              })}
        ...
        <section className="min-h-screen">
          {filteredMovies && movies ? (
            <>
              <div
                className={`flex flex-wrap gap-4 py-8 px-4 mx-auto 
                  ${
